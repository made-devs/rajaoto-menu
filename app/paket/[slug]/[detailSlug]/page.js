...
export default function PackageDetailPage() {
  ...
  // 2. Map package category ke gallery category
  const categoryMap = {
    ac: 'ac',
    kakikaki: 'kakikaki',
    detailing: 'detailing',
  ...
  const galleryCategory = categoryMap[packageDetail?.category] || null;
  const galleryImages = galleryCategory
    ? getImagesByCategory(galleryCategory)
    : [];
  ...
  return (
    <>
      ...
      >
        ...
        {/* --- BAGIAN BARU: GALLERY SECTION --- */}
        {galleryImages.length > 0 && (
          <section className="px-6 mt-10 animate-up">
            <h3 className="text-lg font-bold mb-4 flex items-center gap-2">
              <ShieldCheck className="w-5 h-5 text-yellow-500" />
              Galeri Pengerjaan
            </h3>
            ...
            {galleryImages.length > 6 && (
              <button className="mt-4 w-full text-sm font-semibold text-yellow-500 hover:text-yellow-400 py-2 border border-zinc-700 rounded-lg hover:border-yellow-500 transition-colors">
                Lihat Semua Galeri ({galleryImages.length})
              </button>
            )}
          </section>
        )}
      ...
      {/* --- PORTAL LIGHTBOX (Render ke document.body) --- */}
      {mounted &&
        isLightboxOpen &&
        createPortal(
          <div
            className="fixed inset-0 z-[9999] bg-black/95 backdrop-blur-sm flex justify-center items-center overflow-hidden"
            onClick={() => setIsLightboxOpen(false)}
          >
            {/* Wrapper untuk Lock Ukuran Mobile (max-w-md) */}
            <div className="relative w-full max-w-md h-screen flex flex-col items-center justify-center p-4">
              {/* Close Button */}
              <button
                className="absolute top-6 right-6 bg-zinc-900/80 backdrop-blur border border-zinc-700 p-2 rounded-full text-zinc-200 hover:bg-yellow-500 hover:text-black hover:border-yellow-500 transition-all z-50 shadow-lg group"
                onClick={(e) => {
                  e.stopPropagation();
              ...
              >
                <img
                  src={selectedImage?.image || packageDetail.image}
                  alt={selectedImage?.title || packageDetail.title}
                  className="w-full max-h-[80vh] object-contain rounded-xl border border-zinc-800 shadow-2xl shadow-black"
                />
              </div>

              {/* Caption */}
              <p className="mt-4 text-center text-zinc-400 text-sm font-medium uppercase tracking-widest">
                {selectedImage?.title || packageDetail.title}
              </p>
            </div>
          </div>,
          document.body
